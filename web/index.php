...
$app->post('/api/post/detail/{id}/addComment', function (Request $request, $id) use($app){
    ...
    $lastID = $app['db']->lastInsertId();
    $sql = "SELECT * FROM comments WHERE idPost = ? ORDER BY id DESC"; //the id = ? --> is more secure in the url and it looks nicer
    //$sql = "SELECT * FROM `comments` WHERE idPost = ? ORDER BY id DESC
    $comment_post = $app['db']->fetchAssoc($sql, array((int) $lastID));
